...
object DataPacketSpec {
  val ALAW_RTP_PACKET_SAMPLE: Array[Byte] = Array(0x80, 0x88, 0x19, 0x73, 0x00, 0x01, 0x95, 0x14,
    0x1f, 0xcc, 0x77, 0x9a, 0xd5, 0xd5, 0xd5, 0xd5, 0xd5, 0xd5).map(_.toByte)
  ...
  val H263_PACKET = "80a2123800130ecf4fbc4ca10040000000009842d8ceb128ab51a2ec38ada901890329" +
...
class DataPacketSpec extends WordSpec with MustMatchers {
  import DataPacketSpec._
  "DataPacketSpec" should {
    ...
    "testEncode" in {
      ...
      packet.setData(Array(0xd5, 0xd5, 0xd5, 0xd5, 0xd5, 0xd5).map(_.toByte));
      val buffer = packet.encode();
      ALAW_RTP_PACKET_SAMPLE must equal(buffer.array())
    }
    ...
    "testEncodeDecode" in {
      ...
      packet.setSsrc(96);
      packet.setExtensionHeader(0x8080.toShort, Array(0x70, 0x70, 0x70, 0x70).map(_.toByte));
      packet.addContributingSourceId(69);
      packet.addContributingSourceId(70);
      packet.addContributingSourceId(71);
      packet.setData(Array(0x69, 0x69, 0x69, 0x69).map(_.toByte));
      ...
      packet.getContributingSourcesCount() must equal(decoded.getContributingSourcesCount());
      packet.getContributingSourceIds().get(0) must equal(decoded.getContributingSourceIds().get(0));
      packet.getContributingSourceIds().get(1) must equal(decoded.getContributingSourceIds().get(1));
      packet.getContributingSourceIds().get(2) must equal(decoded.getContributingSourceIds().get(2));
      packet.getDataSize() must equal(decoded.getDataSize());
      packet.getDataAsArray() must equal(decoded.getDataAsArray())
    }

    "testDecodeH263Packet" in {
      val h263packet = ByteUtils.convertHexStringToByteArray(H263_PACKET);
    ...
    "testEncodeDecodeWithFixedBlockSize" in {
      val packet = new DataPacket();
      packet.setMarker(true);
      packet.setSsrc(0x45);
      ...
      packet.setData(Array(0x45, 0x45, 0x45, 0x45, 0x45).map(_.toByte));
      println("packet = " + packet);

      val encoded = packet.encode(64);
      println(ByteUtils.writeArrayAsHex(encoded.array(), true));
      64 must equal(encoded.readableBytes());

      val decoded = DataPacket.decode(encoded);
